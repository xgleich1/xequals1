...
class OutOfPositiveDashOperationToPositiveDashOperationMapperTest {
    @Test
    fun `Should only alter the type of the source & target side`() {
        // GIVEN
        val sourceSide: PositiveDashOperation = mock()
        val targetSide: PositiveDashOperation = mock()
    ...
    private fun map(sourceSide: Term, targetSide: Term) =
            OutOfPositiveDashOperationToPositiveDashOperationMapper.map(sourceSide, targetSide)
}