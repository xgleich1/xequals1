...
class OutOfNegativeDashOperationToPositiveDashOperationMapperTest {
    @Test
    fun `Should only alter the type of the source & target side`() {
        // GIVEN
        val sourceSide: NegativeDashOperation = mock()
    ...
    private fun map(sourceSide: Term, targetSide: Term) =
            OutOfNegativeDashOperationToPositiveDashOperationMapper.map(sourceSide, targetSide)
}