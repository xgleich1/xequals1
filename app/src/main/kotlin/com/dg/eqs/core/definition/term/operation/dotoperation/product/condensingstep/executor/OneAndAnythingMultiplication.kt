...
object OneAndAnythingMultiplication : CondensingExecutor<Terms, Terms> {
    override fun execute(left: Terms, right: Terms): Terms {
        val negativeOneCount = left.count(Term::isNegative)

        return if(negativeOneCount.isEven) right else right.timesNegativeOne()
    }

    private fun Terms.timesNegativeOne() = termsOf(first.invert(), drop(1))
}